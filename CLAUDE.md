...
## Ce qui vit ici
- `skills/` + `.claude-plugin/` — plugin `avqn-dev` distribué par la marketplace auto-hébergée `manu-bernard/avqn-dev`. Installé en scope user → auto-enabled dans chaque session, sans config par repo. Contient `brainstorm-issue` (phase 1), `dev` (phase 2), `apercu` (qualité visuelle) + superpowers vendorisées. **Les wrappers sont les points d'entrée** ; ils orchestrent superpowers.
...
- Onboarder un repo au dev continu = +1 ligne dans `projects.txt`.
